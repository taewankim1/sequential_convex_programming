...
class Scvx_tf_free(Scvx):
    ...
    def cvxopt_del(self):
        ...
        dx_hat_cvx = cvx.Variable((N+1,ix))
        du_cvx = cvx.Variable((N+1,iu))

        x_cvx,u_cvx = [],[]
        dx,dx_hat,du = [],[],[]
        for i in range(N+1) :
            x_cvx.append(self.x[i] + Sx@dx_cvx[i]+sx)
            u_cvx.append(self.u[i] + Su@du_cvx[i]+su)
            dx.append(Sx@dx_cvx[i]+sx)
            du.append(Su@du_cvx[i]+su)
            dx_hat.append(Sx@dx_hat_cvx[i]+sx)

        bf = cvx.Variable((N,10))
        bf_b = cvx.Variable((2,ix))
        sigma = cvx.Variable(nonneg=True)

        # initial & final boundary condition
        constraints = []
        constraints.append(x_cvx[0] + bf_b[0] == self.xi)
        constraints += self.const.bc_final(x_cvx[-1] + bf_b[-1],self.xf)
        constraints.append(dx_hat[0] == np.zeros(ix))
        constraints.append(dx_hat[-1] == dx[-1])

        # state and input contraints
        for i in range(0,N+1) :
            h = self.const.forward(x_cvx[i],u_cvx[i],self.x[i],self.u[i],i==N)
            constraints += h
        ...
        for i in range(0,N) :
            if self.type_discretization == 'zoh' :
                ...
                constraints.append(dx_hat[i+1] == self.A[i]@dx[i] + self.Bm[i]@du[i]
                                                                + self.Bp[i]@du[i+1]
                                                                + (sigma*S_sigma-self.tf)*self.s[i]
                                                                # + vc[i]
                                                                )     
                if i > 0 :
                    constraints.append(dx_hat[i]+self.x_prop[i] == dx[i] + self.x[i])
        ...
        for i in range(0,N+1) :
            ...
            objective_tr.append( self.w_tr * (cvx.quad_form(dx_cvx[i],np.eye(ix)) + cvx.quad_form(du_cvx[i],np.eye(iu))))
        ...
        try :
            ...
            for i in range(N+1) :
                x_bar[i] = x_cvx[i].value
                u_bar[i] = u_cvx[i].value
