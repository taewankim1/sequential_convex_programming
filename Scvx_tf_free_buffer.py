...
class Scvx_tf_free_buffer(Scvx):
    ...
    def cvxopt(self):
        ...
        constraints = []
        constraints.append(Sx@x_cvx[0]+sx+bf_b[0]  == self.xi)
        constraints.append(Sx@x_cvx[-1]+sx+bf_b[-1]  == self.xf)

        # state and input contraints
        for i in range(0,N+1) :
            h = self.const.forward_buffer(Sx@x_cvx[i]+sx,Su@u_cvx[i]+su,bf[i])
            constraints += h
        ...
        objective_buffer.append( self.w_bf * (cvx.quad_form(bf_b[0],np.eye(ix))))
        objective_buffer.append( self.w_bf * (cvx.quad_form(bf_b[1],np.eye(ix))))
        objective_tr.append(1e2 * cvx.norm(sigma-self.tf/S_sigma,2)**2)
        ...
        #                                                         np.min(u_cvx.value),
        #                                                         np.max(u_cvx.value)))
        return prob.status,l.value,l_bf.value,l_tr.value,x_bar,u_bar,sigma_bar,error
    ...
    def run(self,x0,u0,xi,xf,u_const=None):
        ...
        for iteration in range(self.maxIter) :
            ...
            # step2. cvxopt
            # try :
            prob_status,l,l_bf,l_tr,self.xbar,self.ubar,self.sigma,error = self.cvxopt()
            if error == True :
                total_num_iter = 1e5
                break
        ...
        return self.xnew,self.unew,self.xbar,self.ubar,self.sigma,total_num_iter,flag_boundary,l,l_bf,l_tr,x_traj,u_traj
